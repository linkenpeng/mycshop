...
<a href="<?php echo trig_func_common::get_uri("user","add");?>">添加</a>
</div>
  ...
  <form action="<?php echo trig_func_common::get_uri();?>" method="get">
	  ...
	  <table width="100%">
		...
		<tr>
		  ...
		  <?php if($_SESSION['usertype']==ADMIN_USER_TYPE) {?>
		  <td align="center">
				<a href="<?php echo trig_func_common::get_uri("user","edit","admin");?>&uid=<?php echo $value['uid']; ?>" ><?php echo trig_func_common::lang("action","edit")?></a>
				<?php if($value['uid']!=1) {?>
				<a href="<?php echo trig_func_common::get_uri("user","delete","admin");?>&uid=<?php echo $value['uid']; ?>" onclick="return confirm('<?php echo trig_func_common::lang("action","isdelete")?>?');"><?php echo trig_func_common::lang("action","delete")?></a>
				<?php }?>
		  </td>
		  <?php }?>
		</tr>
