...
<div class="pageContent">
  <form action="<?php echo get_uri();?>" method="get">
	  ...
	  <tr>
		...
		型号:<input name="model" type="text" size="20" value="<?=$model?>" />
		条形码:<input name="barcode" type="text" size="20" value="<?=$barcode?>" />
	  ...
	  <table width="100%">
		...
		<tr>
		  ...
		  <td align="center"><?=$producttype_list[$value['typeid']]?></td>
		  ...
		  <td align="center">
				<a href="<?php echo get_uri("product","edit","admin");?>&productid=<?=$value['productid']?>" ><?=lang("action","edit")?></a>
				<a href="<?php echo get_uri("product","delete","admin");?>&productid=<?=$value['productid']?>" onclick="return confirm('<?=lang("action","isdelete")?>?');"><?=lang("action","delete")?></a>
		  </td>
		</tr>
		<?php }} ?>
		<tr>
