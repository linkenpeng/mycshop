...
<!--
	$(function(){
		...
		$("#title").formValidator({onshow:"请输入公告标题",onfocus:"请输入公告标题",oncorrect:"输入正确"}).inputValidator({min:1,onerror:"至少一个字符"});
    ...
    <tr>
      ...
      <td width="80%">
		<select name="notetypeid" id="notetypeid">
			<?php foreach ($notetype_list as $k=>$v) { ?>
				<option value="<?php echo $v['notetypeid'] ?>" <?php if($v['notetypeid']==$value['notetypeid']) echo "selected='selected'";?>><?php echo $v['name'] ?></option>
			<?php } ?>
		</select>
	  (*)</td>
    </tr>
