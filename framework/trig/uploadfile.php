...
class trig_uploadfile {
	...
	/*
	 * 功能：检测并组织文件 $form 文件域名称 $file 上传文件保存名称，为空或者上传多个文件时由系统自动生成名称
	 */
	function upload($form, $file = "") {
		...
		if (!file_exists($this->savepath)) {
			if (!$this->make_dir($this->savepath)) {
				return $this->halt('创建图片目录失败');
			}
		}
	...
	/*
	 * 功能: 设置文件保存名 $savename 保存名，如果为空，则系统自动生成一个随机的文件名
	 */
