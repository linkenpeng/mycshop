...
class producttype {
    ...
    function __construct() {
        ...
        $this->producttypedb = Base::load_model("producttype_model");
    }
    function init() {
        $where = " WHERE 1 ";
        //分页       
        ...
        $show_zone = 1;
        include admin_template('producttype');
    }
    /**
     * 添加
     * 
    ...
    public function add() {
        if (!empty($_POST['action'])) {
            ...
            if ($this->producttypedb->insert($data)) {
                ShowMsg(lang('message','insert_success'),get_uri("producttype","init"));
            } else {
                ShowMsg(lang('message','insert_failure'),-1);
            }
        }
        $show_validator = 1;
        include admin_template('producttypeform');
    }
    /**
     * 修改
     * 
    ...
    public function edit() {
        ...
        if (!empty($_POST['action'])&&!empty($_POST['typeid'])) {
            ...
            if ($this->producttypedb->update($data,"typeid=".$_POST['typeid'])) {
                ShowMsg(lang('message','update_success'),get_uri("producttype","init"));
            } else {
    ...
    public function delete() {
        ...
        if (!empty($typeid)) {
            if ($this->producttypedb->delete($typeid)) {
                ShowMsg(lang('message','delete_success'),get_uri("producttype","init"));
            } else {
