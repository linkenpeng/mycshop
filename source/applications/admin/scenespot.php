...
class scenespot extends controller {
	...
	function __construct() {
		parent::__construct();
		$this->scenespotdb = new scenespot_model();
	}
	function init() {
		$where = " WHERE 1 ";
		...
		include admin_template('scenespot');
	}
	...
	public function add() {
		if (!empty($_POST['action'])) {
			...
			if ($this->scenespotdb->insert($data)) {
				ShowMsg(lang('message', 'insert_success'), get_uri("scenespot", "init"));
			} else {
		...
		include admin_template('scenespotform');
	}
	...
	public function edit() {
		...
		if (!empty($_POST['action']) && !empty($_POST['scenespotid'])) {				
			...
			if ($this->scenespotdb->update($data, "scenespotid=" . $_POST['scenespotid'])) {
				ShowMsg(lang('message', 'update_success'), get_uri("scenespot", "init"));
			} else {
	...
	public function delete() {
		...
		if (!empty($scenespotid)) {
			if ($this->scenespotdb->delete($scenespotid)) {
				ShowMsg(lang('message', 'delete_success'), get_uri("scenespot", "init"));
			} else {
