...
class scenetype extends controller {
    private $scenetypedb;
    function __construct() {
        parent::__construct();
        $this->scenetypedb = new scenetype_model();
    }
    function init() {
        $where = " WHERE 1 ";
        //分页       
        ...
        include admin_template('scenetype');
    }
    /**
     * 添加
     * 
    ...
    public function add() {
        if (!empty($_POST['action'])) {
            ...
            if ($this->scenetypedb->insert($data)) {
                ShowMsg(lang('message','insert_success'),get_uri("scenetype","init"));
            } else {
                ShowMsg(lang('message','insert_failure'),-1);
            }
        }
        $show_validator = 1;
        include admin_template('scenetypeform');
    }
    /**
     * 修改
     * 
    ...
    public function edit() {
        ...
        if (!empty($_POST['action'])&&!empty($_POST['typeid'])) {
            ...
            if ($this->scenetypedb->update($data,"typeid=".$_POST['typeid'])) {
                ShowMsg(lang('message','update_success'),get_uri("scenetype","init"));
            } else {
    ...
    public function delete() {
        ...
        if (!empty($typeid)) {
            if ($this->scenetypedb->delete($typeid)) {
                ShowMsg(lang('message','delete_success'),get_uri("scenetype","init"));
            } else {
