...
class relic extends controller {
    ...
    function init() {
		...
		$levels = array(1=>'一级',2=>'二级',3=>'三级');
        include admin_template('relic');
    }
    /**
     * 添加
     * 
    ...
    public function add() {
        if (!empty($_POST['action'])) {
            ...
            if ($this->relicdb->insert($data)) {
                ShowMsg(lang('message','insert_success'),get_uri("relic","init"));
            } else {
        ...
        include admin_template('relicform');
    }
    /**
     * 修改
     * 
    ...
    public function edit() {
        ...
        if (!empty($_POST['action'])&&!empty($_POST['relicid'])) {
            ...
            if ($this->relicdb->update($data,"relicid=".$_POST['relicid'])) {
                ShowMsg(lang('message','update_success'),get_uri("relic","init"));
            } else {
    ...
    public function delete() {
        ...
        if (!empty($relicid)) {
            if ($this->relicdb->delete($relicid)) {
                ShowMsg(lang('message','delete_success'),get_uri("relic","init"));
            } else {
