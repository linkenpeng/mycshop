<?php
defined('SYS_IN') or exit('Access Denied.');
class traveltopic {
    private $traveltopicdb;
    function __construct() {
        //判断是否登录
        Base::load_model("login_model")->is_login();
        $this->traveltopicdb = Base::load_model("traveltopic_model");
    }
    function init() {
        $where = " WHERE 1 ";
        //分页       
        Base::load_sys_class("page",'',0);
        $count = $this->traveltopicdb->get_count($where);
        $pagesize = !isset($_GET['pagesize']) ? "15" : $_GET['pagesize'];
        $nowpage = isset($_GET['page']) ? intval($_GET['page']) : 1;
        $setarr = array(
            'total'=>$count,
            'perpage'=>$pagesize
        );
        $p = new page($setarr);
        //获取分页后的数据
        $list = $this->traveltopicdb->get_list($pagesize,$pagesize*($nowpage-1)," * ",$where,"dateline DESC ");
        $show_zone = 1;
        include admin_template('traveltopic');
    }
    /**
     * 添加
     * 
     * @author Myron
     * 2011-5-27 上午11:57:59
     */
    public function add() {
        if (!empty($_POST['action'])) {
            if (empty($_POST['name'])) {
                ShowMsg("分类名不能为空!",-1);
            }
            $_POST['dateline'] = empty($_POST['dateline']) ? time() : strtotime(trim($_POST['dateline']));
            if (!empty($_FILES['image']['name'])) {
                Base::load_sys_class("upfile",'',0);
                $upfile = new upfile("jpg,gif,bmp,png");
                $upfile->savesamll = 1;
                $image = $upfile->upload($_FILES['image']);
            }
			$data = array(
                'parentid'=>$_POST['parentid'],
                'uid'=>$_SESSION['admin_uid'],
                'username'=>$_SESSION['admin_username'],
                'name'=>$_POST['name'],
				'image'=>$image,
                'description'=>$_POST['description'],
                'dateline'=>$_POST['dateline']
            );
            if ($this->traveltopicdb->insert($data)) {
                ShowMsg(lang('message','insert_success'),get_uri("traveltopic","init"));
            } else {
                ShowMsg(lang('message','insert_failure'),-1);
            }
        }
        $show_validator = 1;
        include admin_template('traveltopicform');
    }
    /**
     * 修改
     * 
     * @author Myron
     * 2011-5-27 上午09:36:34
     */
    public function edit() {
        $typeid = $_GET['typeid'];
        if (!empty($typeid)) {
            $value = $this->traveltopicdb->get_one($typeid);
        }
        if (!empty($_POST['action'])&&!empty($_POST['typeid'])) {
            if (!empty($_FILES['image']['name'])) {
                Base::load_sys_class("upfile",'',0);
                $upfile = new upfile("jpg,gif,bmp,png");
                $upfile->savesamll = 1;
                $image = $upfile->upload($_FILES['image']);
            }
			$data = array(
                'name'=>$_POST['name'],
                'description'=>$_POST['description']
            );
			if (!empty($image)) {
                $data['image'] = $image;
                //删除老图片
                if($image!=$_POST['oldimage']) {
                    @unlink(UPLOAD_PATH.'/'.$_POST['oldimage']);
                    @unlink(UPLOAD_PATH.'/thumb/'.$_POST['oldimage']);
                }
            }
            if ($this->traveltopicdb->update($data,"typeid=".$_POST['typeid'])) {
                ShowMsg(lang('message','update_success'),get_uri("traveltopic","init"));
            } else {
                ShowMsg(lang('message','update_failure'),-1);
            }
        }
        $show_validator = 1;
        include admin_template('traveltopicform');
    }
    /**
     * 删除
     * 
     * @author Myron
     * 2011-5-27 上午09:36:34
     */
    public function delete() {
        $typeid = $_GET['typeid'];
        if (!empty($typeid)) {
            if ($this->traveltopicdb->delete($typeid)) {
                ShowMsg(lang('message','delete_success'),get_uri("traveltopic","init"));
            } else {
                ShowMsg(lang('message','delete_failure'),-1);
            }
        } else {
            ShowMsg(lang('message','param_error'),-1);
        }
    }
}
?>