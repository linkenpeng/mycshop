...
class talkreply_model extends model {
     ...
     * 
     * 获取一条信息
     * @param int $talkreplyid
     */
    function get_one($talkreplyid = "") {
       if(!empty($talkreplyid)) {
			$where = "where talkreplyid=".$talkreplyid;
     ...
     * return Boolean $flag
     */
    function delete($talkreplyid) {
        $flag = false;
        $sql = "DELETE FROM ".tname($this->table)." WHERE talkreplyid=".$talkreplyid;
