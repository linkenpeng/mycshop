...
class notetype_model extends model {
     ...
     */
    function get_one($notetypeid = "") {
       if(!empty($notetypeid)) {
			$where = "where notetypeid=".$notetypeid;
     ...
     * return Boolean $flag
     */
    function delete($notetypeid) {
        $flag = false;
        $sql = "DELETE FROM ".tname($this->table)." WHERE notetypeid=".$notetypeid;
