...
function __autoload($class_name) {
	//自动包含模型类
	if (file_exists ( ROOT_PATH .DS . "source" . DS . 'model' . DS . $class_name . '.php' )) {
      ...
      function JumpUrl(){
        ...
        if ($onlymsg==0) {
            if ($gourl!="javascript:;"&&$gourl!="") {
                $rmsg .= "<br/><br/><a href='".$gourl."'>".lang("message","messagenote")."</a>";
            }
            $rmsg .= "<br/><br/></div>\");\r\n";
            if ($gourl!="javascript:;"&&$gourl!="") {
...
*return 语音包中的值
*/
function lang($langfile, $key) {
    global $_CTCONFIG;
    if (empty($langfile) or empty($key)) {
        return "Undefined Language!";
    } else {
        if (file_exists(ROOT_PATH.DS."source".DS."languages".DS.$_CTCONFIG['lang'].DS.$langfile.".php")) {
            include ROOT_PATH.DS."source".DS."languages".DS.$_CTCONFIG['lang'].DS.$langfile.".php";
        }
        return $lang[$key];
    }
}
 ...
 * $template 模板目录
 */
function template($name, $templatedir = '') {
    global $_CTCONFIG;
    ...
    if (!file_exists($objfile)||filemtime($objfile)<filemtime(ROOT_PATH.DS.'./'.$tpl.'.htm')) {
        $tpl_parse = Base::load_sys_class("template");
        $tpl_parse->parse_template($tpl,$templatedir);
    }
    return $objfile;
}
//子模板更新检查
function subtplcheck($subfiles, $mktime, $tpl, $templatedir) {
...
function sgmdate($dateformat, $timestamp = '', $format = 0) {
    ...
    if ($format) {
        ...
        if ($time>24*3600) {
            ...
            $result = intval($time/60).lang('datetime','minute').lang('datetime','before');
        } elseif ($time>0) {
            $result = $time.lang('datetime','second').lang('datetime','before');
        } else {
...
function cn_substr($str, $length = 0, $append = false, $charset = '', $istrimhtml = 1) {
    global $_G;
    $str = trim($str);
    if (!empty($istrimhtml)) {
        $str = trimhtml($str);
...
function showResult($v) {
    if ($v==1) {
        echo '<b>√</b>&nbsp;<font class=green>'.lang('common','support').'</font>';
    } else {
        echo '<font class=red><b>×</b></font>&nbsp;<font class=red>'.lang('common','unsupport').'</font>';
    }
}
//获取路由
function get_uri($c = '', $a = '', $m = '', $extra = '') {
    $route_config = Base::load_config('route');
...
//ajax 分页函数
function ajax_page($num, $perpage, $curpage, $mpurl, $ajaxdiv, $waitmsg) {
    $maxpage = 10;
    $page = 5;
    $multipage = '';
    $mpurl .= strpos($mpurl,'?') ? '&' : '?';
...
function getPinyin($str,$ishead=0,$isclose=1) {
	...
	if(count($pinyins)==0) {
		$fp = fopen(ROOT_PATH."/source/languages/zh-cn/pinyin.dat",'r');
		while(!feof($fp)) {
			$line = trim(fgets($fp));
...
function debugEx($var,$isdump = false) {
	echo '<pre>';
	if($isdump) {
		var_dump($var);
	} else {
